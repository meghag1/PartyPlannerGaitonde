...
public class Party {
  ...
  Person[][] tabSet;
  public ArrayList<Person> namesList = new ArrayList<Person>(); // creation of array list
  // make an array list for each table

  public int table = 10; // 10 tables
  public int numppl = 10;
  public int numgleft = 10;
  public int tablepos = 10; // positions at each table

  /*
   * add person method - this method prompts users if they choose to add a new
   * person to the roster
   * it take in the info and should not allow them to add the user if there is
   * already 10 ppl of that company - counter starts at 0 and everytime there is
   * already a person wtih that comapny id it adds to the counter
  ...
  /*
   * the look person method - this method will prompt the user for the first name
   * of the person they are looking for and then report back the info of that
   * person
   * it return their name, company, table, and seat
   */
  ...
  public int lookPerson() {
    ...
    for (Person guest : namesList) {
      if (guest.getFName().equals(search)) {
        ...
        System.out.println("\nTable: " + (guest.getTable() + 1));// need to find a way to also print out table number
                                                                 // and position
        System.out.println("Seat: " + (guest.getposID() + 4));
        return 0;
      } // clsoe if search
  ...
  /*
   * method to load the data file into the program and also parse and put it into
   * an array list (namesList) as it reads every line
   */
  ...
  /*
   * set up method will create the 2d array that will represent the 10 tables and
   * 10 table positions and populate it with the Person objects
   */
  ...
  public void SetUp() {
    ...
    for (int s = 0; s < tablepos; s++) { // change these
      for (int t = 0; t < table; t++) {
        ...
        // this is here to check that two people from the same company are not at the
        // same table
        // if they are it will remove them and have a empty seat (null)
        for (int j = 0; j < tablepos; j++) {

          if (tabSet[t][j] != null && tabSet[t][j].getComp() == namesList.get(i).getComp()) {
  ...
  /*
   * print all tables method will print the 10 tables and each person in it while
   * also printing seat number in front of each person
   * It itterates through and for each table number goes through all the people in
   * it and prints them
   */
  ...
  /*
   * ask table will ask the user what table they want to look up the roster for
   * and based on that print out the table and all its people and their seats
   */
  ...
  /*
   * compRoster method prints out a roster of all the people from a compant that
   * the user inputs
   */

  public void compRoster() {
    System.out.print("Here are the companies in attenedence at this event: 1)Wal-Mart 2) Kroger 3) Amazon 4) Lowes 5) Best Western 6) KMart 7) Fusian 8) Heinz 9) Gucci 10) Prada 11) Nike 12) Dodge 13) Maserati 14) Razor 15) AMD 16) Razer");
    System.out.print("What Company would you like to print a roster for (1-15): ");
    Scanner scan = new Scanner(System.in);
    int comproster = scan.nextInt();
    System.out.println("\nHere is all the atendees from company " + comproster);
    for (Person guest : namesList) {
